...
//        }
    ...
    */
    ...
    private void startCameraX(ProcessCameraProvider cameraProvider) {
        ...
        // 카메라 바인딩
        cameraProvider.bindToLifecycle((LifecycleOwner) (AppCompatActivity) getActivity(), cameraSelector, preview, videoCapture);
    ...
    public void startRecordVideo() {
        ...
        Toast.makeText(getActivity().getApplicationContext(), "동영상 녹화가 시작되었습니다.", Toast.LENGTH_SHORT).show();

        File newPath = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
        File photoDir = new File(newPath, "CameraXMovies_jung/");
        ...
        String vidFilePath = photoDir.getAbsolutePath() + "/" + "JJ_" + timeStamp + ".mp4";
        ...
        videoCapture.startRecording( // error : videoCapture = null
            new VideoCapture.OutputFileOptions.Builder(vidFile).build(),
            ContextCompat.getMainExecutor(getActivity().getApplicationContext()),
            new VideoCapture.OnVideoSavedCallback() {
                @Override
                public void onVideoSaved(@NonNull VideoCapture.OutputFileResults outputFileResults) {
                     Toast.makeText(context, "동영상 녹화가 저장되었습니다.", Toast.LENGTH_SHORT).show();
                }

                @Override
                public void onError(int videoCaptureError, @NonNull String message, @Nullable Throwable cause) {
                    Toast.makeText(getActivity().getApplicationContext(), "Error " + videoCaptureError + "  message:" + message + "   cause:" + cause, Toast.LENGTH_SHORT).show();
                }
            }
        );
    }
